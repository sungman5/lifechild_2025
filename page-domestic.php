<?php get_header(); ?>
<main id="domestic-page">
    <section style="display: none">
        <div class="wrapper8">
            <header class="section-header">
                <h2 class="section-header-headline">국내사업</h2>
                <p class="section-header-cat">국내 소외계층을 위한 따뜻한 동행, 희망을 만드는 국내사업</p>
            </header>
            <div class="section-hero">
                <img src=<?php echo get_template_directory_uri() ?>/img/photo-by-Yonghua-Fu-on-unsplash.jpg" alt=""
                     title="Photo-by-傅甬 华(Yonghua Fu) on unsplash">
            </div>
        </div>
    </section>
    <section class="">
        <div class="wrapper8">
            <header class="section-header">
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000" viewBox="0 0 256 256">
                    <path d="M249.88,92.47l-120-64a4,4,0,0,0-3.76,0l-120,64a4,4,0,0,0,0,7.06L36,115.47v50.82a11.92,11.92,0,0,0,3,8c12.65,14.09,41,37.73,89,37.73,21,0,38.2-4.52,52-10.79V240a4,4,0,0,0,8,0V197.18a111.64,111.64,0,0,0,29-22.91,11.94,11.94,0,0,0,3-8V115.47l29.88-15.94a4,4,0,0,0,0-7.06ZM128,204c-44.83,0-71.25-22-83-35.08a3.92,3.92,0,0,1-1-2.63V119.73l82.12,43.8a4,4,0,0,0,3.76,0L180,136.8v55.53C166.58,199,149.39,204,128,204Zm84-37.71a4,4,0,0,1-1,2.64,103.32,103.32,0,0,1-23,19v-55.4l24-12.8Zm-24.59-42.51a4,4,0,0,0-1.53-1.44l-56-29.87a4,4,0,0,0-3.76,7.06L179.5,128,128,155.47,16.5,96,128,36.53,239.5,96Z"></path>
                </svg>
                <h2 class="section-header-headline">교육비 지원사업</h2>
                <p class="section-header-cat">
                    경제적 어려움으로부터 아이들의 배움과 관계, 자존감을 지켜주는 지원사업
                </p>

            </header>
            <div class="domestic-container">
                <div class="domestic">
                    <img src=<?php echo get_template_directory_uri() ?>/img/photo-by-jerry-Wang-on-unsplash.jpg" alt=""
                         title="Photo by jerry Wang on unsplash">
                    <div class="domestic-content">
                        <div class="domestic-number-grid">
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    23
                                </p>
                                <p class="domestic-stats-caption">
                                    협력기관 수
                                </p>
                            </div>
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    872
                                </p>
                                <p class="domestic-stats-caption">
                                    누적 수혜 인원(2025년 기준)
                                </p>
                            </div>
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    32,000,000
                                </p>
                                <p class="domestic-stats-caption">
                                    누적 지원금액(단위: 원)
                                </p>
                            </div>
                        </div>
                        <p class="domestic-excerpt">
                            <strong>모든 아이는 배울 권리가 있습니다.</strong> 하지만 경제적 어려움은 그 권리를 흔들고, 아이를 점점 배움에서 멀어지게 만듭니다.
                            때로는 학용품 하나, 친구들과 다니고 싶은 학원 하나가 아이의 교우관계와 자존감에 영향을 주기도 합니다. 라이프오브더칠드런은 아이들이 배움에서 소외되지 않도록,
                            학업뿐 아니라 친구와의 관계, 자존감까지 지켜주는 교육비 지원을 이어가고 있습니다.
                            <strong>교육은 단지 공부를 위한 것이 아니라, 아이의 내일과 연결된 모든 가능성의 시작이기 때문입니다.</strong>
                        </p>
                        <ul>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M146.7,166.75a36,36,0,1,0-37.4,0A63.61,63.61,0,0,0,76.8,189.6a4,4,0,0,0,6.4,4.8,56,56,0,0,1,89.6,0,4,4,0,0,0,6.4-4.8A63.65,63.65,0,0,0,146.7,166.75ZM100,136a28,28,0,1,1,28,28A28,28,0,0,1,100,136ZM200,28H56A12,12,0,0,0,44,40V216a12,12,0,0,0,12,12H200a12,12,0,0,0,12-12V40A12,12,0,0,0,200,28Zm4,188a4,4,0,0,1-4,4H56a4,4,0,0,1-4-4V40a4,4,0,0,1,4-4H200a4,4,0,0,1,4,4ZM92,64a4,4,0,0,1,4-4h64a4,4,0,0,1,0,8H96A4,4,0,0,1,92,64Z"></path>
                                </svg>
                                <div>
                                    <h4>지원대상</h4>
                                    <p>라이프오브더칠드런은 정관에 근거하여 중위소득 75%이하의 ①이주노동자-다문화 가정 아동 ②북한이탈주민 가정 아동 ③한부모 가정 아동 ④기타
                                        소외계층 가정
                                        아동에게 생계비를 일부 지원하고 있습니다.</p>
                                </div>
                            </li>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M100,104a4,4,0,0,1,4-4h64a4,4,0,0,1,0,8H104A4,4,0,0,1,100,104Zm4,36h64a4,4,0,0,0,0-8H104a4,4,0,0,0,0,8Zm124,52a28,28,0,0,1-28,28H88a28,28,0,0,1-28-28V64a20,20,0,0,0-40,0c0,7.78,6.34,12.75,6.4,12.8a4,4,0,1,1-4.8,6.4C21.21,82.91,12,75.86,12,64A28,28,0,0,1,40,36H176a28,28,0,0,1,28,28V172h12a4,4,0,0,1,2.4.8C218.79,173.09,228,180.14,228,192Zm-120,0c0-7.78-6.34-12.75-6.4-12.8A4,4,0,0,1,104,172h92V64a20,20,0,0,0-20-20H59.57A27.9,27.9,0,0,1,68,64V192a20,20,0,0,0,40,0Zm112,0c0-6-3.74-10.3-5.5-12H112.61A23.31,23.31,0,0,1,116,192a27.94,27.94,0,0,1-8.42,20H200A20,20,0,0,0,220,192Z"></path>
                                </svg>
                                <div>
                                    <h4>지원내용</h4>
                                    <p>자격 확인 및 심사를 거쳐, 최소 1년부터 최대 3년까지 생계비를 소액 지원합니다.</p>
                                </div>
                            </li>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M128,28A100,100,0,1,0,228,128,100.11,100.11,0,0,0,128,28Zm92,100a91.67,91.67,0,0,1-24.21,62.13L65.87,60.21A92,92,0,0,1,220,128ZM36,128A91.67,91.67,0,0,1,60.21,65.87L190.13,195.79A92,92,0,0,1,36,128Z"></path>
                                </svg>
                                <div>
                                    <h4>제한사항</h4>
                                    <p>라이프오브더칠드런은 개인의 직접 신청은 받지 않습니다. 인근 복지관이나 이주민지원센터 등 관련 기관을 통해 지원을 요청해주세요.</p>
                                </div>
                            </li>
                        </ul>
                        <div class="page-donation-banner"
                             style="background-image: url('<?php echo get_template_directory_uri() ?>/img/photo-by-jerry-Wang-2-on-unsplash.jpg')">
                            <div class="page-donation-banner-content">
                                <h4 class="page-donation-banner-content-white">후원안내</h4>
                                <p class="page-donation-banner-content-white">
                                    볼펜 하나, 학원 하나가<br>
                                    아이의 마음을 바꿉니다.
                                </p>
                            </div>
                            <a href="" class="page-donation-banner-button">
                                매월 1만 원 후원하기
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section class="gray-bg">
        <div class="wrapper8">
            <div class="section-header">
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000" viewBox="0 0 256 256">
                    <path d="M216,160a8,8,0,1,1-8-8A8,8,0,0,1,216,160Zm-4.17,35.79A44.06,44.06,0,0,1,168,236H144a44.05,44.05,0,0,1-44-44V147.85A60,60,0,0,1,44,88V40a4,4,0,0,1,4-4H72a4,4,0,0,1,0,8H52V88a52,52,0,0,0,52.69,52C133,139.63,156,115.93,156,87.17V44H136a4,4,0,0,1,0-8h24a4,4,0,0,1,4,4V87.17c0,32-24.84,58.59-56,60.69V192a36,36,0,0,0,36,36h24a36.05,36.05,0,0,0,35.8-32.25,36,36,0,1,1,8,0ZM236,160a28,28,0,1,0-28,28A28,28,0,0,0,236,160Z"></path>
                </svg>
                <h2 class="section-header-headline">
                    의료비 지원사업
                </h2>
                <p class="section-header-cat">치료받을 수 있는 권리를 지키기 위해, 아이들의 긴급한 의료비를 지원합니다.</p>
            </div>
            <div class="domestic-container">
                <div class="domestic">
                    <img src=<?php echo get_template_directory_uri() ?>/img/photo-by-Aditya-Romansa-on-unsplash.jpg" alt=""
                         title="Photo-by-傅甬 华(Yonghua Fu) on unsplash">
                    <div class="domestic-content">
                        <div class="domestic-number-grid">
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    23
                                </p>
                                <p class="domestic-stats-caption">
                                    협력기관 수
                                </p>
                            </div>
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    872
                                </p>
                                <p class="domestic-stats-caption">
                                    누적 수혜 인원(2025년 기준)
                                </p>
                            </div>
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    32,000,000
                                </p>
                                <p class="domestic-stats-caption">
                                    누적 지원금액(단위: 원)
                                </p>
                            </div>
                        </div>
                        <p class="domestic-excerpt">
                            <strong>아이에게 아픔은 견뎌야 할 일이 아니라, 치유받아야 할 일입니다.</strong> 그러나 많은 아이들이 치료비 부담으로 병원 문턱조차 넘지 못하고,
                            고통을 감내하고 있습니다. 가정 형편 때문에 치료를 미루거나 포기하는 일이 더는 반복되지 않도록, 라이프오브더칠드런은 긴급하고 꼭 필요한 의료비를 지원합니다.
                            <strong>의료는 선택이 아닌 기본권이며, 건강하게 자랄 수 있는 환경은 모든 아이가 누려야 할 최소한의 권리입니다.</strong>
                            우리는 후원자님과 함께 아이들의 아픔을 함께 나누고, 건강한 내일로 이어지는 길을 만들어갑니다.
                        </p>
                        <ul>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M146.7,166.75a36,36,0,1,0-37.4,0A63.61,63.61,0,0,0,76.8,189.6a4,4,0,0,0,6.4,4.8,56,56,0,0,1,89.6,0,4,4,0,0,0,6.4-4.8A63.65,63.65,0,0,0,146.7,166.75ZM100,136a28,28,0,1,1,28,28A28,28,0,0,1,100,136ZM200,28H56A12,12,0,0,0,44,40V216a12,12,0,0,0,12,12H200a12,12,0,0,0,12-12V40A12,12,0,0,0,200,28Zm4,188a4,4,0,0,1-4,4H56a4,4,0,0,1-4-4V40a4,4,0,0,1,4-4H200a4,4,0,0,1,4,4ZM92,64a4,4,0,0,1,4-4h64a4,4,0,0,1,0,8H96A4,4,0,0,1,92,64Z"></path>
                                </svg>
                                <div>
                                    <h4>지원대상</h4>
                                    <p>라이프오브더칠드런은 정관에 근거하여 중위소득 75%이하의 ①이주노동자-다문화 가정 아동 ②북한이탈주민 가정 아동 ③한부모 가정 아동 ④기타
                                        소외계층 가정
                                        아동에게 생계비를 일부 지원하고 있습니다.</p>
                                </div>
                            </li>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M100,104a4,4,0,0,1,4-4h64a4,4,0,0,1,0,8H104A4,4,0,0,1,100,104Zm4,36h64a4,4,0,0,0,0-8H104a4,4,0,0,0,0,8Zm124,52a28,28,0,0,1-28,28H88a28,28,0,0,1-28-28V64a20,20,0,0,0-40,0c0,7.78,6.34,12.75,6.4,12.8a4,4,0,1,1-4.8,6.4C21.21,82.91,12,75.86,12,64A28,28,0,0,1,40,36H176a28,28,0,0,1,28,28V172h12a4,4,0,0,1,2.4.8C218.79,173.09,228,180.14,228,192Zm-120,0c0-7.78-6.34-12.75-6.4-12.8A4,4,0,0,1,104,172h92V64a20,20,0,0,0-20-20H59.57A27.9,27.9,0,0,1,68,64V192a20,20,0,0,0,40,0Zm112,0c0-6-3.74-10.3-5.5-12H112.61A23.31,23.31,0,0,1,116,192a27.94,27.94,0,0,1-8.42,20H200A20,20,0,0,0,220,192Z"></path>
                                </svg>
                                <div>
                                    <h4>지원내용</h4>
                                    <p>자격 확인 및 심사를 거쳐, 최소 1년부터 최대 3년까지 생계비를 소액 지원합니다.</p>
                                </div>
                            </li>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M128,28A100,100,0,1,0,228,128,100.11,100.11,0,0,0,128,28Zm92,100a91.67,91.67,0,0,1-24.21,62.13L65.87,60.21A92,92,0,0,1,220,128ZM36,128A91.67,91.67,0,0,1,60.21,65.87L190.13,195.79A92,92,0,0,1,36,128Z"></path>
                                </svg>
                                <div>
                                    <h4>제한사항</h4>
                                    <p>라이프오브더칠드런은 개인의 직접 신청은 받지 않습니다. 인근 복지관이나 이주민지원센터 등 관련 기관을 통해 지원을 요청해주세요.</p>
                                </div>
                            </li>
                        </ul>
                        <div class="page-donation-banner"
                             style="background-image: url('<?php echo get_template_directory_uri() ?>/img/photo-by-Chen-Yij-on-unsplash.jpg')">
                            <div class="page-donation-banner-content">
                                <h4>후원안내</h4>
                                <p>
                                    건강하게 자랄 수 있는 권리,<br>
                                    여러분이 지켜주세요.
                                </p>
                            </div>
                            <a href="" class="page-donation-banner-button">
                                매월 1만 원 후원하기
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <section>
        <div class="wrapper8">
            <div class="section-header">
                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000" viewBox="0 0 256 256">
                    <path d="M216,76H166.81a50.54,50.54,0,0,0,12.63-8A25.53,25.53,0,0,0,188,49.44,28.62,28.62,0,0,0,158.56,20,25.53,25.53,0,0,0,140,28.56c-5.91,6.67-9.63,15.36-12,23.69-2.35-8.33-6.07-17-12-23.69A25.53,25.53,0,0,0,97.44,20,28.62,28.62,0,0,0,68,49.44,25.53,25.53,0,0,0,76.56,68a50.54,50.54,0,0,0,12.63,8H40A12,12,0,0,0,28,88v32a12,12,0,0,0,12,12h4v68a12,12,0,0,0,12,12H200a12,12,0,0,0,12-12V132h4a12,12,0,0,0,12-12V88A12,12,0,0,0,216,76ZM146,33.86A17.59,17.59,0,0,1,158.8,28h.61A20.62,20.62,0,0,1,180,49.2,17.6,17.6,0,0,1,174.14,62c-12.09,10.7-33.07,13.21-42,13.79C132.76,66.93,135.26,46,146,33.86ZM76,49.2A20.62,20.62,0,0,1,96.59,28h.61A17.59,17.59,0,0,1,110,33.86c10.71,12.09,13.21,33.07,13.79,42-8.89-.58-29.87-3.09-42-13.79A17.6,17.6,0,0,1,76,49.2ZM36,120V88a4,4,0,0,1,4-4h84v40H40A4,4,0,0,1,36,120Zm16,80V132h72v72H56A4,4,0,0,1,52,200Zm152,0a4,4,0,0,1-4,4H132V132h72Zm16-80a4,4,0,0,1-4,4H132V84h84a4,4,0,0,1,4,4Z"></path>
                </svg>
                <h2 class="section-header-headline">
                    희망나눔사업
                </h2>
                <p class="section-header-cat">명절과 계절 고비마다 소외된 이웃에게 따뜻함과 필수 물품을 전하는 희망나눔사업입니다.</p>
            </div>
            <div class="domestic-container">
                <div class="domestic">
                    <img src=<?php echo get_template_directory_uri() ?>/img/photo-by-lc-winter.jpg" alt=""
                         title="Photo-by-傅甬 华(Yonghua Fu) on unsplash">
                    <div class="domestic-content">
                        <div class="domestic-number-grid">
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    23
                                </p>
                                <p class="domestic-stats-caption">
                                    협력기관 수
                                </p>
                            </div>
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    872
                                </p>
                                <p class="domestic-stats-caption">
                                    누적 수혜 인원(2025년 기준)
                                </p>
                            </div>
                            <div class="domestic-number-item">
                                <p class="domestic-stats">
                                    32,000,000
                                </p>
                                <p class="domestic-stats-caption">
                                    누적 지원금액(단위: 원)
                                </p>
                            </div>
                        </div>
                        <p class="domestic-excerpt">
                            <strong>기념일은 모두에게 따뜻해야 하니까요.</strong> 희망나눔사업은 추석과 어린이날 같은 특별한 날은 물론, 혹서기·혹한기, 김장철, 연탄 나눔 등
                            계절의 고비마다
                            소외된 이웃에게 꼭 필요한 물품과 마음을 전하는 라이프오브더칠드런의 연중 지원활동입니다. <strong>누군가에게는 사소할 수 있는 하루가, 다른 누군가에겐 버텨야
                                할 하루가 되지 않도록.</strong> 우리는 작은 선물과 계절의 따뜻함으로 이웃들의 일상에 희망을 더합니다.
                        </p>
                        <ul>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M146.7,166.75a36,36,0,1,0-37.4,0A63.61,63.61,0,0,0,76.8,189.6a4,4,0,0,0,6.4,4.8,56,56,0,0,1,89.6,0,4,4,0,0,0,6.4-4.8A63.65,63.65,0,0,0,146.7,166.75ZM100,136a28,28,0,1,1,28,28A28,28,0,0,1,100,136ZM200,28H56A12,12,0,0,0,44,40V216a12,12,0,0,0,12,12H200a12,12,0,0,0,12-12V40A12,12,0,0,0,200,28Zm4,188a4,4,0,0,1-4,4H56a4,4,0,0,1-4-4V40a4,4,0,0,1,4-4H200a4,4,0,0,1,4,4ZM92,64a4,4,0,0,1,4-4h64a4,4,0,0,1,0,8H96A4,4,0,0,1,92,64Z"></path>
                                </svg>
                                <div>
                                    <h4>지원대상</h4>
                                    <p>라이프오브더칠드런은 정관에 근거하여 중위소득 75%이하의 ①이주노동자-다문화 가정 아동 ②북한이탈주민 가정 아동 ③한부모 가정 아동 ④기타
                                        소외계층 가정
                                        아동에게 생계비를 일부 지원하고 있습니다.</p>
                                </div>
                            </li>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M100,104a4,4,0,0,1,4-4h64a4,4,0,0,1,0,8H104A4,4,0,0,1,100,104Zm4,36h64a4,4,0,0,0,0-8H104a4,4,0,0,0,0,8Zm124,52a28,28,0,0,1-28,28H88a28,28,0,0,1-28-28V64a20,20,0,0,0-40,0c0,7.78,6.34,12.75,6.4,12.8a4,4,0,1,1-4.8,6.4C21.21,82.91,12,75.86,12,64A28,28,0,0,1,40,36H176a28,28,0,0,1,28,28V172h12a4,4,0,0,1,2.4.8C218.79,173.09,228,180.14,228,192Zm-120,0c0-7.78-6.34-12.75-6.4-12.8A4,4,0,0,1,104,172h92V64a20,20,0,0,0-20-20H59.57A27.9,27.9,0,0,1,68,64V192a20,20,0,0,0,40,0Zm112,0c0-6-3.74-10.3-5.5-12H112.61A23.31,23.31,0,0,1,116,192a27.94,27.94,0,0,1-8.42,20H200A20,20,0,0,0,220,192Z"></path>
                                </svg>
                                <div>
                                    <h4>지원내용</h4>
                                    <p>자격 확인 및 심사를 거쳐, 최소 1년부터 최대 3년까지 생계비를 소액 지원합니다.</p>
                                </div>
                            </li>
                            <li class="domestic-detail">
                                <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" fill="#000000"
                                     viewBox="0 0 256 256">
                                    <path d="M128,28A100,100,0,1,0,228,128,100.11,100.11,0,0,0,128,28Zm92,100a91.67,91.67,0,0,1-24.21,62.13L65.87,60.21A92,92,0,0,1,220,128ZM36,128A91.67,91.67,0,0,1,60.21,65.87L190.13,195.79A92,92,0,0,1,36,128Z"></path>
                                </svg>
                                <div>
                                    <h4>제한사항</h4>
                                    <p>라이프오브더칠드런은 개인의 직접 신청은 받지 않습니다. 인근 복지관이나 이주민지원센터 등 관련 기관을 통해 지원을 요청해주세요.</p>
                                </div>
                            </li>
                        </ul>
                        <div class="page-donation-banner"
                             style="background-image: url('<?php echo get_template_directory_uri() ?>/img/photo-by-melanie-rosillo-galvan-on-unsplash.jpg')">
                            <div class="page-donation-banner-content">
                                <h4>후원안내</h4>
                                <p>
                                    함께하는 기념일,<br>함께하는 계절
                                </p>
                            </div>
                            <a href="" class="page-donation-banner-button">
                                매월 1만 원 후원하기
                            </a>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <nav class="floating-page-menu">
        <div class="floating-menu-accordion">
            <?php
            wp_nav_menu([
                'theme_location' => 'business',
                'container' => false,
                'menu_class' => 'floating-menu-list'
            ])

            ?>
            <div class="floating-menu-current-page">
                <p><?php the_title() ?></p>
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="#000000" viewBox="0 0 256 256">
                    <path d="M210.83,162.83a4,4,0,0,1-5.66,0L128,85.66,50.83,162.83a4,4,0,0,1-5.66-5.66l80-80a4,4,0,0,1,5.66,0l80,80A4,4,0,0,1,210.83,162.83Z"></path>
                </svg>
            </div>
        </div>
        <ul class="floating-page-menu-list">
            <li class="floating-page-menu-item">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#000000" viewBox="0 0 256 256">
                    <path d="M249.88,92.47l-120-64a4,4,0,0,0-3.76,0l-120,64a4,4,0,0,0,0,7.06L36,115.47v50.82a11.92,11.92,0,0,0,3,8c12.65,14.09,41,37.73,89,37.73,21,0,38.2-4.52,52-10.79V240a4,4,0,0,0,8,0V197.18a111.64,111.64,0,0,0,29-22.91,11.94,11.94,0,0,0,3-8V115.47l29.88-15.94a4,4,0,0,0,0-7.06ZM128,204c-44.83,0-71.25-22-83-35.08a3.92,3.92,0,0,1-1-2.63V119.73l82.12,43.8a4,4,0,0,0,3.76,0L180,136.8v55.53C166.58,199,149.39,204,128,204Zm84-37.71a4,4,0,0,1-1,2.64,103.32,103.32,0,0,1-23,19v-55.4l24-12.8Zm-24.59-42.51a4,4,0,0,0-1.53-1.44l-56-29.87a4,4,0,0,0-3.76,7.06L179.5,128,128,155.47,16.5,96,128,36.53,239.5,96Z"></path>
                </svg>
                <p>교육비 지원</p>
            </li>
            <li class="floating-page-menu-item">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#000000" viewBox="0 0 256 256">
                    <path d="M216,160a8,8,0,1,1-8-8A8,8,0,0,1,216,160Zm-4.17,35.79A44.06,44.06,0,0,1,168,236H144a44.05,44.05,0,0,1-44-44V147.85A60,60,0,0,1,44,88V40a4,4,0,0,1,4-4H72a4,4,0,0,1,0,8H52V88a52,52,0,0,0,52.69,52C133,139.63,156,115.93,156,87.17V44H136a4,4,0,0,1,0-8h24a4,4,0,0,1,4,4V87.17c0,32-24.84,58.59-56,60.69V192a36,36,0,0,0,36,36h24a36.05,36.05,0,0,0,35.8-32.25,36,36,0,1,1,8,0ZM236,160a28,28,0,1,0-28,28A28,28,0,0,0,236,160Z"></path>
                </svg>
                <p>의료비 지원</p>
            </li>
            <li class="floating-page-menu-item">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="#000000" viewBox="0 0 256 256">
                    <path d="M216,76H166.81a50.54,50.54,0,0,0,12.63-8A25.53,25.53,0,0,0,188,49.44,28.62,28.62,0,0,0,158.56,20,25.53,25.53,0,0,0,140,28.56c-5.91,6.67-9.63,15.36-12,23.69-2.35-8.33-6.07-17-12-23.69A25.53,25.53,0,0,0,97.44,20,28.62,28.62,0,0,0,68,49.44,25.53,25.53,0,0,0,76.56,68a50.54,50.54,0,0,0,12.63,8H40A12,12,0,0,0,28,88v32a12,12,0,0,0,12,12h4v68a12,12,0,0,0,12,12H200a12,12,0,0,0,12-12V132h4a12,12,0,0,0,12-12V88A12,12,0,0,0,216,76ZM146,33.86A17.59,17.59,0,0,1,158.8,28h.61A20.62,20.62,0,0,1,180,49.2,17.6,17.6,0,0,1,174.14,62c-12.09,10.7-33.07,13.21-42,13.79C132.76,66.93,135.26,46,146,33.86ZM76,49.2A20.62,20.62,0,0,1,96.59,28h.61A17.59,17.59,0,0,1,110,33.86c10.71,12.09,13.21,33.07,13.79,42-8.89-.58-29.87-3.09-42-13.79A17.6,17.6,0,0,1,76,49.2ZM36,120V88a4,4,0,0,1,4-4h84v40H40A4,4,0,0,1,36,120Zm16,80V132h72v72H56A4,4,0,0,1,52,200Zm152,0a4,4,0,0,1-4,4H132V132h72Zm16-80a4,4,0,0,1-4,4H132V84h84a4,4,0,0,1,4,4Z"></path>
                </svg>
                <p>희망나눔사업</p>
            </li>
        </ul>
    </nav>
</main>
<?php get_footer(); ?>
